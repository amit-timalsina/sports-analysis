...
class CRUDRepository(Generic[ModelType, CreateSchemaType, ReadSchemaType, UpdateSchemaType]):
    ...
    async def read(
        ...
        """
        return await self.read_by_filter(self.model.id == record_id, current_user)  # type: ignore[arg-type]
    ...
    async def read_by_filter(
        ...
        result = await session.execute(
            select(self.model).filter(filter_condition).filter(self.user_filter(current_user)),
        )
    ...
    async def read_multi(
        ...
        session = await self.get_session()
        query = (
            select(self.model).filter(self.user_filter(current_user)).offset(offset).limit(limit)
        )
    ...
    async def count(self, current_user: User | None = None) -> int:
        """
        ...
        session = await self.get_session()
        result = await session.execute(
            select(func.count(self.model.id)).filter(self.user_filter(current_user)),
        )
