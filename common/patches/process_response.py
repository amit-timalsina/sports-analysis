...
async def process_response_async(
    ...
    *,
    ...
    if (
        ...
        return await response_model.from_streaming_response_async(
            response,
            mode=mode,
        )
    ...
    if isinstance(model, IterableBase):
        logger.debug("Returning takes from IterableBase")
...
def handle_json_o1(
    ...
    if "system" in roles:
        msg = "System messages are not supported For the O1 models"
...
def handle_json_modes(
    ...
    if new_kwargs["messages"][0]["role"] != "system":
        ...
        msg = "Invalid message format, must be a string or a list of messages"
        raise ValueError(
            msg,
        )
...
def handle_vertexai_parallel_tools(
    response_model: type[Iterable[T]],
    new_kwargs: dict[str, Any],
...
def handle_cerebras_tools(
    ...
    if new_kwargs.get("stream", False):
        msg = "Stream is not supported for Cerebras Tool Calling"
        raise ValueError(msg)
    new_kwargs["tools"] = [
        {
            "type": "function",
...
"""
    ...
    new_kwargs["messages"] = [{"role": "system", "content": instruction}] + new_kwargs["messages"]
    return response_model, new_kwargs
...
def handle_response_model(
    ...
    **kwargs: Any,
    ...
    if mode in {Mode.VERTEXAI_PARALLEL_TOOLS}:
